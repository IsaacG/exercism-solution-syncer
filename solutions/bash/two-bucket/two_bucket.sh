...
# will *always* *always* print 2. This is one of the dangers of using this notation.
# This occurs because b=0 evaluates to 0 and (( 0 )) is false.
# `true && false || cmd` evaluates (true && false), gets a false and then runs cmd.
# I had to work around this by ensuring that when I use `cond && (( ... )) || (( ... ))`,
# the last value inside the first `(( ... ))` must never be 0.
# The "correct" solution would be to use `if cond; then (( ... )); else (( ... )); fi`
...
# If that leaves us water in both buckets, don't do anything else. Otherwise...
...
# Note, it's expected that this function is called with one bucket (partially/fully) filled and the other empty.
fill_pour_empty_append () {
  local b1=$1 b2=$2 moves=$3 swap=$4 c no_fill_pour=0
  # Rather than using b1|b2, use full|empty volume|capacity, fv ev fc ec.
  # It makes maths a lot simpler.
  local -i fv fc ev ec
  (( b1 )) && (( fv=b1, fc=capacity1, ev=0, ec=capacity2 )) || (( fv=b2, fc=capacity2, ev=0, ec=capacity1 ))

  # Pour from one bucket into the other prior to filling and pouring.
  if (( swap )); then
    if (( fv > ec )); then # Full bucket volume does not fit fully in then empty bucket.
      (( ev=ec, fv-=ev, moves++ ))
      (( no_fill_pour++ ))
    else
...
buckets () {
  ...
  # Initial conditions
  [[ $start = 'one' ]] && (( b2=0, b1=capacity1 )) || (( b1=0, b2=capacity2 ))
  # Pre-check if we are done before we start.
  check_complete $b1 $b2 1
  check_complete $capacity1 $b2 2
  check_complete $b1 $capacity2 2

  # We did not yet explore anything. This line isn't strictly needed.
  for (( i = 0; i <= capacity1 || i <= capacity2; i++ )); do explored[i]=0; done
  ...
  while (( "${#to_explore[@]}" )); do
    ...
    # If we saw this one already, don't do it a second time. Otherwise, mark it as seen.
  ...
  # Nothing left to explore! We've tried all the reachable scenarios and what's left is bleak.
  echo "invalid goal"
  exit 1
}
